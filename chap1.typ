...
其中极小极大值为$v_(1 m), v_(2 m)$：在$v_(1 m) < v < v_(2 m)$的范围内，$(pdv(p, v))_T > 0$，不满足平衡稳定性条件的要求，这些状态是不可能作为均匀系而实现的
...
- 根据Gibbs判据，在给定的$T$、$P$下，系统的稳定平衡状态是使Gibbs函数最小的状态，从而$O K B A M R$上各点代表系统的稳定平衡状态，而$A J D N$上各点代表系统的不稳定状态

#figure(
  image("pic/2026-03-16-22-50-53.png", width: 40%),
  numbering: none,
)
