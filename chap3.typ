...
低温极限有$theta_E / T >> 1$，分立性必须考虑，*热容*为
$
  C_V approx 3 N k (theta_E/T)^2 e^(- theta_E/T)
$
与实验定性相符，定量不符$prop T^3$，原因是实际上，有*低频模*($nu tilde C/lambda$长声学波)，它们在低温下仍然可以被激发，因此对热容有贡献(Debye)；而Einstein模型设定$nu = nu_i$，等于把所有模都放在同一个有限频率上，于是低温时它们几乎都激发不了。

== 顺磁物质的磁性

顺磁物质是指在外磁场存在时，能被磁化的物质。它们的磁性来自于原子或分子中的未成对电子自旋或轨道角动量。
$
  m = chi H, chi = m/H = (mu_0 m)/B_0
$
其中$m$为*磁化强度*，$chi$为*磁化率*，$H$为*外磁场强度*。对于顺磁物质，磁化率$chi$是正的，且通常随温度升高而减小。真空中的磁感应强度是
$
  B_0 = mu_0 H
$
其中$mu_0 = 4 pi times 10^(-7)$H/m，是真空中的磁导率；材料中的磁感应强度
$
  B = mu_0 (H + m) = mu_0 (1 + chi) H = mu B_0
$
其中$mu = 1 + chi$为*磁导率*。

磁性分类为
- 顺磁：$chi > 0$，$mu > 1$，满足Curie定律$chi prop C/T$
- 逆磁：$chi < 0$，$mu < 1$，$tilde 10^(-5)$

顺磁体*没有自发磁化*，外场一撤掉，热运动很快把取向打乱，宏观磁化消失。

有些材料即使没有外磁场，内部磁矩也会自己排出有序结构，就叫自发磁化。
- 铁磁体：磁矩平行排，出现净磁化
  - 例如Fe, Co, Ni
  - 相邻原子的磁矩由于交换作用，倾向于平行排列：$arrow.t arrow.t arrow.t arrow.t arrow.t$
  - 现实里一块铁不一定总表现出很大磁化：分成很多磁畴，每个磁畴内部磁矩大致平行，不同磁畴方向可能不同
- 反铁磁体：两套磁矩反平行，净磁化为零
  - 例如#ch("FeO")
  - 两套子晶格磁矩等大反向排列：$arrow.t arrow.b arrow.t arrow.b$
- 亚铁磁体：反平行排列，但不能完全抵消
  - 例如#ch("Fe3O4")
  - 两套子晶格磁矩不等大反向排列：$arrow.t arrow.b arrow.t arrow.b arrow.t$
铁磁（反铁磁、亚铁磁）都有*顺磁相变*，随着温度升高，热运动越来越强，最终会把原来的自发有序破坏掉。

*顺磁物质*其分子（或原子，离子）具有恒定的磁矩$mu$。当$H=0$时，热运动使得总磁化$M=0$；当$H!=0$时，外场使这些磁矩倾向沿$H$排列，但热运动又在破坏这种排列，于是最后形成竞争平衡。
$
  vb(mu) = mu_B g vb(J)
$
其中
...
$
是*Lande因子*，$vb(J)$是总角动量。对于单原子分子，磁矩来自于未成对电子的自旋和轨道角动量；对于分子，磁矩还可能来自于核自旋。

=== 顺磁气体与顺磁晶体的磁性

顺磁气体由顺磁性粒子构成，例如#ch("O2")、#ch("NO")等；顺磁晶体由顺磁性离子构成，例如络钾矾中#ch("Cr^3+")等。*磁化过程与其他运动近似独立*，因此可以单独处理；如果磁性粒子之间相互作用可以忽略，那么就可以用近独立粒子统计。

用Boltzmann分布
- *顺磁气体*：由顺磁性粒子构成，粒子*不可区分*
- *顺磁晶体*：晶体中含顺磁离子，粒子*可区分*

单个磁性粒子能级
$
  epsilon_(m_i) = - vb(mu) dot vb(B) = - mu_B g vb(J) dot vb(B) = - mu_0 mu_B g m_i H
$
其中$mu_0$是真空磁导率，$mu_B$是Bhor磁子，$g$是Lande因子，$m_i$是总角动量的磁量子数，即$vb(J)$在$vb(H)$方向上的分量，取值为$-j, -j+1, ..., j-1, j$，总共有$2 j + 1$个能级。

从而单个磁性粒子*配分函数*为
$
  Z(beta, H) &= sum_(m_i=-j)^j e^(- beta epsilon_(m_i)) = (e^(- a(j + 1/2)) - e^(a(j + 1/2))) / (e^(-a/2) - e^(a/2)) = (e^(- a j) (1 - e^(- a (2 j + 1)))) / (1 - e^(- a)) \
  & = sinh((j + 1/2) a) / sinh(a/2)
$
其中
$
  a = beta mu_0 mu_B g H
$
进一步可以得到$J$沿$H$取$m_i$的粒子数由Boltzmann分布给出
$
  N(m_i) = e^(- alpha - beta epsilon_(m_i)) = N/Z e^(- beta epsilon_(m_i))
$
*总粒子数*
$
  N = sum N(m_i) = sum e^(-alpha - beta epsilon_(m_i)) = e^(- alpha) Z(beta, H)
$
*总磁化强度*就是各能级磁矩贡献的求和
$
  M & = sum_(m_i=-j)^j N(m_i) mu_B g m_i = N/Z sum_(m_i=-j)^j mu_B g m_i e^(- a m_i) \
    & = N/beta pdv(ln Z, mu_0 H) = N mu_B g ((j + 1/2) coth(a(j + 1/2)) - 1/2 coth(a/2)) = N mu_B g j B_j (a)
$
其中$B_j (a)$为*Brillouin函数*；即总磁化$=$每个粒子的最大磁矩$×$粒子数$×$平均定向程度

#figure(
  image("pic/3.1.png", width: 80%),
  numbering: none,
)

- *单位体积磁化强度*
  $
    m = M/V = n mu_B g j B_j (a)
  $
  其中$n$是单位体积粒子数，也可以写为
  $
    m = n/beta pdv(ln Z, mu_0 H)
  $
  - *高$T$弱$H$极限*

    $a<<1$，有
    $
      coth(x) = 1/x + x/3 ==> B_j (a) = 1/3a(j+1)
    $
    得到
    $
      m = C H/T
    ...
    其中
    $
      C = 1/3 j(j+1) mu_0 (mu_B g)^2 n / k_B
    $
    是*Curie常数*，满足*Curie定律*。
  - *低$T$强$H$极限*
    ...
    $
      m = n mu_B g j
    $
    几乎所有磁矩都与$H$同向，*磁饱和*：外场已经强到，或者温度已经低到，几乎所有粒子都排到最低能方向去了。
- 在恒磁场条件下的*比热*
  $
    Z(beta, H) = sinh((j + 1/2) a) / sinh(a/2)
  $
  从而
  $
    E & = - N pdv(ln Z, beta) \
      & = - N mu_0 mu_B g H ((j + 1/2) coth(a(j + 1/2)) - 1/2 coth(a/2)) \
      & = - N mu_0 mu_B g H j B_j (a)
  $
  $
    C_B = (pdv(E, T))_B
  $
  #figure(
    image("pic/3.2.png", width: 80%),
    numbering: none,
  )
- *绝热去磁* Adiabatic Demagnetization
  $
    Z(beta, H) = sum_(m_i = -j)^j e^(- mu_0 mu_B g m_i H beta) = sinh((j + 1/2) mu_0 mu_B g H beta) / sinh((1/2) mu_0 mu_B g H beta)
  $
  可以得到自由能
  $
    F = - N/beta ln Z = 1/beta phi(beta H)
  $
  而熵为
  $
    S = - pdv(F, T) = k_B (- phi(beta H) + beta H phi'(beta H))
  $
  *绝热过程*中，$S$不变，从而$beta H = H/(k_B T)$不变，即$T prop H$，外场越弱，温度越低。

  若绝热条件下缓慢降低磁场$H$，为了保持熵不变，就必须同时降低温度$T$。因此，绝热去磁可以用来制冷，尤其是制备超低温。
  $
    T_f = T_i H_f/H_i
  $

  要选择$S(H=0)$随温度变化小的物质，例如顺磁盐#ch("Ce2Mg3(NO3)12·(H2O)24")这类物质常用于极低温制冷，可冷却到 0.01K以下。

== 负绝对温度

#note[
  1951年, Pursell&Pound实现了负绝对温度的状态，很纯的LiF晶体的核自旋系统中

  1956年，Ramsey 提出了有关负绝对温度的热力学与统计理论。
]
...
$
  1/T = (pdv(S, U))_V
$
- 一般：内能愈高，可能的微观状态数愈多，因此$S$随$U$增加而增加，$T>0$
- 反常：内能愈高，可能的微观状态数愈少，因此$S$随$U$增加而减少，$T<0$；这就是*负绝对温度*
也就是：温度的符号由熵对内能的斜率决定。

*核自旋在外磁场中的两能级系统*是负绝对温度的典型例子。考虑一个孤立的核自旋系统，参数是：粒子数$N$，能量$E$，外磁场$B$。

核自旋量子数为$1/2$，在$vb(B)$中
$
  Delta E = plus.minus mu_N B = plus.minus epsilon
$
对应反平行和平行两种取向
- 平行于磁场：低能态
- 反平行于磁场：高能态
设$N_+$为能量为$epsilon$的核自旋数，$N_-$为能量为$- epsilon$的核自旋数，则
$
  N_- + N_+ = N
$
*系统的能量*
...
将*熵用能量表示*，即粒子数用能量表示
$
  N_+ = N/2 (1 + E/(N epsilon)), N_- = N/2 (1 - E/(N epsilon))
$
则有
$
  S & = k (N ln N - N_+ ln N_+ - N_- ln N_-) \
    & = N k (ln 2 - 1/2 (1 + E/(N epsilon)) ln (1 + E/(N epsilon)) - 1/2 (1 - E/(N epsilon)) ln (1 - E/(N epsilon)))
$
所以熵随能量的变化会呈现：从低能端开始，先升高到中间附近最大，再往高能端走，又下降；从而温度
$
  1/T = (pdv(S, E))_V = - k/(2 epsilon) ln ((N epsilon - E)/(N epsilon + E))
$
- $E<0$时，$T<0$
- $E=0$时，$T=oo$
- $E>0$时，$T>0$
...
#figure(
  image("pic/3.3.pdf", width: 80%),
  numbering: none,
)
温度的顺序：$0^+ -> + oo -> - oo -> 0^-$，即负温态比任意正温态都更热。

下面讨论什么条件下可以把整个体系分为核自旋系统和晶格系统，并实现核自旋系统的负绝对温度？*实现负绝对温度的条件*
- 系统能量必须有上界
- 系统内部实现平衡（系统能与环境隔绝一段时间）
  - 自旋系统内部彼此交换能量很快，所以它自己能先达到统计平衡
  - 相当于*系统本身达到平衡的弛予时间远小于系统与环境之间达到平衡的弛予时间*。
    $
      tau_"spin-spin" << tau_"spin-lattice"
    $
    只有满足以上条件，划分才有意义
    - 对LiF晶体：$tau_"spin-spin" tilde 10^(-5)"s"$，$tau_"spin-lattice" tilde 5 "min"$
